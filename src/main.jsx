  ...
  return <main className="page">
    <header className="hero cinematic-surface">
      ...
      <div className="hero-meta">
        ...
        <a className="button button-ghost" href="/api/admin/export">Export progress</a>
      </div>
    </header>
...
function TrackShell({ trackId, section }) {
  ...
  return <main className={`page track ${trackId}`}>
    ...
    {activeSection === 'learn' && <section className="cards">{cardsDue.map((card) => {
      ...
      return <article className="learning-card cinematic-surface" key={card.id}><p className="eyebrow">{card.track_id} / {card.domain_name}</p><h3>{card.prompt}</h3><p><strong>Short answer:</strong> {card.short_answer}</p><details><summary>Detailed explanation</summary><p>{card.detailed_explanation}</p><p>Sources: {card.source_links.map((s) => <a key={s} href={s}>{new URL(s).hostname}</a>)}</p></details><p className="card-state-row"><strong>Card status:</strong> <span className={`status-pill status-${cardState.status}`}>{cardState.status}</span></p><p><strong>Next review:</strong> {cardState.next_review_at || card.spaced_repetition.due_at}</p>{cardAction.error && cardAction.errorCardId === card.id && <p className="warning" role="alert">{cardAction.error}</p>}<div className="actions"><button className="button button-primary" disabled={isBusy} onClick={() => markCard(card, 'know')}>{isBusy ? 'Saving…' : 'I know this'}</button><button className="button button-secondary" disabled={isBusy} onClick={() => markCard(card, 'review')}>{isBusy ? 'Saving…' : 'Review again'}</button></div></article>;
    })}</section>}
    {activeSection === 'quiz' && <section className="quiz-shell"><Panel title="Quiz engine"><div className="actions"><button className="button button-primary" disabled={quizLoading} onClick={() => startQuiz('quick')}>Quick 10</button><button className="button button-secondary" disabled={quizLoading} onClick={() => startQuiz('domain', data.domains[0].id)}>Domain 15</button><button className="button button-secondary" disabled={quizLoading} onClick={() => startQuiz('full')}>Full 65 timed</button><button className="button button-secondary" disabled={quizLoading} onClick={() => startQuiz('weakness')}>Weakness drill</button><button className="button button-secondary" disabled={quizLoading} onClick={() => startQuiz('mixed')}>Mixed review</button></div>{quizLoading && <p>Loading quiz…</p>}</Panel>{answerError && <p className="warning" role="alert">{answerError}</p>}{quiz && <Quiz quiz={quiz} quizIndex={quizIndex} onAnswer={answer} review={review} reviews={quizReviews} finished={quizFinished} onNext={nextQuestion} onFinish={finishQuiz} onRetry={retryQuiz} answerLoading={answerLoading} />}</section>}
