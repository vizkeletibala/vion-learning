...
function Quiz({ quiz, quizIndex, onAnswer, review, reviews, finished, onNext, onFinish, onRetry, answerLoading }) {
  ...
  return <Panel title={`${quiz.mode} quiz · ${quiz.question_count} questions ${quiz.timed_minutes ? `· ${quiz.timed_minutes} minutes` : ''}`}><p className="eyebrow">Question {quizIndex + 1} of {quiz.question_count}</p><h3>{question.prompt}</h3>{question.options.map((o) => <button disabled={answerLoading || Boolean(review)} key={o.id} onClick={() => onAnswer(question, o)}>{answerLoading ? 'Checking…' : o.label}</button>)}{review && <div className="review"><h3>{review.correct ? 'Correct' : 'Review needed'}</h3><p>{review.correct_explanation}</p><p>Selected: {review.selected_explanation}</p><p>Mapping: {review.mapping.track_id} / domain {review.mapping.domain_id} / card {review.mapping.card_id}</p><p>Next actions: {review.next_actions.join(', ')}</p><button onClick={isFinalQuestion ? onFinish : onNext}>{isFinalQuestion ? 'Finish quiz' : 'Next question'}</button></div>}</Panel>;
}
