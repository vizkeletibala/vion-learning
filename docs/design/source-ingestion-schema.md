...
Store ingested sources as static JSON arrays under the certification that owns them:

- `data/sources/clf-c02/ingested_sources.json`
- `data/sources/aif-c01/ingested_sources.json`

Each file contains records with exactly one `track_id`. Do not create a shared mixed-track source file. A source that is useful to both exams must be represented as two records, one per track, because domains, concepts, summaries, and exam relevance are certification-specific.
...
1. `track_id` is required on every source and must be either `clf-c02` or `aif-c01`.
2. Folder and record track must match: `data/sources/clf-c02/*` may only contain `track_id: "clf-c02"`; `data/sources/aif-c01/*` may only contain `track_id: "aif-c01"`.
...
5. Do not dedupe across tracks by URL. Local JSON can repeat URLs when the interpretation differs by exam.
...
2. Keep each source track-owned. A shared AWS documentation URL may appear twice, but the CLF-C02 and AIF-C01 records must have different ids, exam codes, domains, concepts, and separation notes.
